...
Status CreateUDG(MGraph *G) { /* 采用数组(邻接矩阵)表示法,构造无向图G */
	int i, j, k, l, IncInfo;
	char s[MAX_INFO], *info;
	VertexType va, vb;
...
Status CreateUDN(MGraph *G) { /* 采用数组(邻接矩阵)表示法,构造无向网G。算法7.2 */
	int i, j, k, w, IncInfo;
	char s[MAX_INFO], *info;
	VertexType va, vb;
...
Status CreateGraph(MGraph *G) { /* 采用数组(邻接矩阵)表示法,构造图G。算法7.1 */
